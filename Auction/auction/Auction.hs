...
main = keep $ initNode $ do
  ...
  where
    ...
    auctionSequence = do
      auction <- start 
      waitClose auction <|> public "bids" $ bid auction 
      where
      start= do
        ...
        runPAB $ start startparams
        return startparams

      bid auction= do
        bid <- minput "start" ("currency: " <> currency <> "   token: " <> token "Enter bid parameter" :: String)
        runPAB bid $ BidParams (spCurrency bid) (spToken auction) bid
    
      waitClose auction= do
        wait $ deadLine auction
...
getGuessGameSchema :: AuctionContracts -> [FunctionSchema FormSchema]
