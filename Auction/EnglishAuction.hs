...
import           Plutus.V1.Ledger.Scripts -- ( Datum (Datum),Validator,ValidatorHash,Redeemer(..))
...
typedAuctionValidator = Scripts.mkTypedValidator @Auctioning
  ...
  where
    wrap = Scripts.mkUntypedValidator @AuctionDatum @AuctionAction
...
bid BidParams{..} = do
    ...
    let b  = Bid {bBidder = pkh, bBid = bpBid}
        ...
        lookups = Constraints.plutusV1TypedValidatorLookups typedAuctionValidator P.<>
                  Constraints.plutusV1OtherScript auctionValidator                P.<>
...
findAuction cs tn = do
    ...
    case xs of
        [(oref, o)] -> case  _ciTxOutScriptDatum o of -- ciTxOutDatum o of
            (_,Nothing)          -> throwError "datum missing"
